...
//17.  Progress barfiller
  ...
  // モバイル向け：セクション最下部からのスクロールで次へ移動
  if ($(window).width() <= 767) {
      let startY = 0;
      let isAnimating = false;
      // 対象となるセクションのセレクタ（表示順）
      const sectionsSelector = '.slider-area, .visit-tailor-area, .our-cases-area, .support-company-area, .home-blog-area, footer';
      ...
      $(document).on('touchmove', function(e) {
          ...
          const deltaY = startY - currentY; // 正の値なら下方向へのスクロール（指を上に動かす）
          const scrollTop = $(window).scrollTop();
          const windowHeight = $(window).height();

          // 一定以上のスワイプ操作（30px）があった場合
          if (deltaY > 30) { 
              let currentSection = null;
              
              // 現在表示されている（最下部にある）セクションを特定
              $(sectionsSelector).each(function() {
                  ...
                  // 画面の下端がセクションの下端付近（誤差5px以内）にあるか
                  // かつ、画面の上端がセクションの上端より下にある（セクション内をスクロール済み）
                  if (scrollTop + windowHeight >= bottom - 5 && scrollTop > top) {
                      currentSection = $this;
              ...
              if (currentSection && currentSection.length) {
                  ...
                  const allSections = $(sectionsSelector);
